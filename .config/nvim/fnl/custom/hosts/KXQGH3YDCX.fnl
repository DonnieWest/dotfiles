(set vim.g.lit_analyzer_script
     (vim.fs.joinpath (vim.fn.stdpath :config) :scripts
  ...
  (let [root (platform-root)]
    (if (not root)
        ...
        (do
          ...
          (vim.cmd.lcd (vim.fn.fnameescape (vim.fs.joinpath root :projects
                                                            :people)))
          (let [argv [:yarn]]
...
(local commands {:PeopleStart [:start]
  ...
  (vim.api.nvim_create_user_command name
                                    (fn [opts]
                                      (let [args (vim.deepcopy command)
                                            env (if (= name
                                                       :PeopleStartDevelopment)
                                                    {:ENV :development}
                                                    (= name :PeopleStart)
                                                    {:ENV :staging}
                                                    nil)]
                                        (when (and (not= opts.args "")
                                                   (not env))
